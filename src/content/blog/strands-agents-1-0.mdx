...
travel_advisor_agent = Agent(
    system_prompt="You are a travel expert who helps with trip planning",
...
In this example, we define travel and research agents who have specialized prompts and tools for their areas of focus, which the executive assistant agent can call upon for input on the user's request.
...
# The agent calls the handoff_to_user tool which includes the question
...
# Enables Strands debug logs level
...
researcher = Agent(
    name="researcher",
    system_prompt="You research topics thoroughly using your memory",
    tools=[memory]
)
...
writer = Agent(
    name="writer",
    system_prompt="You write comprehensive reports based on research",
...
These multi-agent patterns are designed to be gradually adopted and freely combined—start with single agents, add specialists as tools, evolve to swarms, and orchestrate with graphs as your needs grow.

## Multi-Agent Systems with A2A

Strands 1.0 includes support for the [Agent-to-Agent (A2A) protocol](https://a2aproject.github.io/A2A/latest/), an open standard that enables agents from different platforms to communicate seamlessly. Any Strands agent can be wrapped with A2A capabilities to become network accessible and adhere to the A2A protocol.
...
Because A2A provides features like the agent card, a standardized description of agent capabilities, A2A-enabled multi-agent systems can easily discover and connect to agents created by other teams or organizations. Learn more about [A2A integration](/docs/user-guide/concepts/multi-agent/agent-to-agent/) in the Strands documentation.

## Production-Ready Features

While Strands has been used in production by Amazon teams like Amazon Q Developer and AWS Glue long before its public release, we've been working backwards with hundreds of customers worldwide to extend Strands to support your production needs.

### Durable Session Management

We've added `SessionManager`, a session management abstraction that enables automatic persistence and restoration of agent conversations and state. This allows agents to save their complete history to a storage backend like Amazon S3 and seamlessly resume conversations even after compute restarts.
...
You can extend this abstraction with your own storage backend implementation. Strands includes local filesystem and Amazon S3 backends by default. Learn more about [Session Management](/docs/user-guide/concepts/agents/session-management/) in the documentation.

### Native Async Support

Production workloads demand reliability and responsive performance. For 1.0, we've improved the Strands event loop architecture to support async operations throughout the entire stack. Tools and model providers can now run asynchronously without blocking, enabling true concurrent execution.
...
Customers told us they needed flexibility to use different models for different tasks. Model providers like Anthropic, Meta, OpenAI, Cohere, Mistral, Stability, and Writer have made contributions which enable their own model API to be used by a Strands Agent. Strands lets you use different models for each agent, or switch models and model providers without modifying your tools or logic.
...
# Use different models for different agents in the same system
researcher = Agent(name="researcher", model=anthropic_model, tools=[web_search])
...
The Strands community has been a critical voice in shaping all of these updates, through usage, feedback and direct code contributions. We're deeply grateful to each of you for helping make Strands the simplest way to take an agent from prototype to production.
