...
Here is a minimal example of an Agent SOP used as a system prompt:

```python
from strands import Agent

agent = Agent(
    system_prompt="""# Code Review SOP
...
:::tip
For example, if the agent follows steps 1 and 2, but gets sidetracked by a comment on an issue, you know that you need to provide more guidance on step 2 in order to better direct the agent's attention.
:::
