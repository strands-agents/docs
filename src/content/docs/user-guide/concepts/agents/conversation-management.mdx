...
The threshold check requires the model's context window size. The SDK auto-populates `contextWindowLimit` from built-in lookup tables for known models (Anthropic, Bedrock, OpenAI, Google Gemini). You can override it manually for models not in the lookup table:
...
If `contextWindowLimit` is not set and the model ID is not in the built-in lookup table, the SDK falls back to a default of 200,000 tokens.
...
- **Actual limit is smaller** (e.g., 8k or 32k): Compression triggers too late. The model will hit context overflow errors before proactive compression is triggered, falling back fully to reactive overflow recovery.
- **Actual limit is larger** (e.g., 1M): Compression triggers too early. The agent proactively trims context well before the model's capacity is reached.
:::

### Token Estimation

The agent estimates input tokens using a the following strategy:

1. **Known baseline**: Reads `inputTokens + outputTokens` from the last assistant message's metadata
