...
The `countTokens()` implementation varies by model provider — some use native token counting APIs for exact counts, while others use a character-based heuristic. See the Token Counting section on each provider's page for details:
...
Providers without native token counting APIs (OpenAI Chat Completions, Ollama, Mistral, LiteLLM, Vercel) use a character-based heuristic (characters ÷ 4 for text, characters ÷ 2 for JSON). All native providers also fall back to this heuristic when native counting is unavailable or disabled.

### Context Window Limit Fallback
...
```
contextWindowLimit is not set on the model, using default of 200000 — set contextWindowLimit in your model config for accurate proactive compression
```

:::caution[Inaccurate compression with default fallback]
When the default 200,000-token limit is used but the model's actual context window is significantly different, proactive compression will not behave correctly:

- **Actual limit is smaller** (e.g., 8k or 32k): Compression triggers too late — the model will hit context overflow errors before proactive compression fires, falling back to reactive recovery.
- **Actual limit is larger** (e.g., 1M): Compression triggers too early — the agent proactively trims context well before the model's capacity is reached, discarding useful conversation history unnecessarily.
:::

To avoid this, always set `contextWindowLimit` explicitly for models not in the built-in lookup tables:
