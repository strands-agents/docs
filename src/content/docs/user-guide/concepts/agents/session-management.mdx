...
`SessionManager` is a [Plugin](../plugins/index.mdx). The `sessionManager` field on `AgentConfig` is a convenience shorthand — it is equivalent to passing it in the `plugins` array:
...
### Multi-Agent Sessions <em>(Python only)</em>
...
## Immutable Snapshots <em>(TypeScript only)</em>
...
## Deleting Sessions <em>(TypeScript only)</em>
