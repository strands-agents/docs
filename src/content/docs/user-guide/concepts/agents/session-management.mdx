...
The TypeScript `SessionManager` uses a `saveLatestOn` strategy to control when `snapshot_latest` is updated:

- **`'invocation'`** *(default)*: Saves after every agent invocation completes. Balances durability and I/O.
- **`'message'`**: Saves after every message added to the conversation. Most durable, highest I/O.
- **`'trigger'`**: Only saves when a `snapshotTrigger` callback fires or when `saveSnapshot` is called manually.

On initialization, the agent automatically restores state from `snapshot_latest` if it exists.

```typescript
--8<-- "user-guide/concepts/agents/session-manager.ts:save_latest_strategy"
```
