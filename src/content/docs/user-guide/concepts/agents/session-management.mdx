...
:::caution
You cannot use a single agent with a session manager inside a multi-agent system. In Python, this will throw an exception. In TypeScript, the orchestrator snapshots and restores each agent node's state on every execution, so an agent-level session manager would conflict with the orchestrator's persistence. Each agent in a multi-agent system must be created without a session manager — only the orchestrator should have one. Additionally, multi-agent session managers only track the current state of the Graph/Swarm execution and do not persist individual agent conversation histories.
:::

:::note[TypeScript]
`SessionManager` implements both [Plugin](../plugins/index.mdx) (for agents) and `MultiAgentPlugin` (for orchestrators). The `sessionManager` field on `Agent`, `Graph`, and `Swarm` is a convenience shorthand for passing it in the `plugins` array.
:::
