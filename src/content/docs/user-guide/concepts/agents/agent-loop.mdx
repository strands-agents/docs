...
Sometimes the loop needs to be stopped from outside — a web server handling a client disconnect, a background task manager responding to changed conditions, or a UI "Stop" button. The `agent.cancel()` method provides a thread-safe way to do this.

Calling `cancel()` sets an internal signal that the agent checks at two checkpoints:

1. During model response streaming — partial output is discarded
2. Before tool execution — pending tool calls are skipped, with error results added to maintain valid conversation state
