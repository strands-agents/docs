...
Sometimes the loop needs to be stopped from outside. A web server handling a client disconnect, a background task manager responding to changed conditions, or a UI "Stop" button are all cases where external cancellation is needed. The `agent.cancel()` method provides a thread-safe way to do this.

Calling `cancel()` sets an internal signal that the agent checks at two checkpoints:

1. During model response streaming, where partial output is discarded. Usage metrics may be inaccurate in this case since the stream is closed before the model sends its final metadata event.
2. Before tool execution, where tool calls are skipped with error results added to maintain valid conversation state
...
<Tabs>
  <Tab label="Python">
    ...
    async def main():
        ...
        result = await agent.invoke_async("Analyze this large dataset")
        watchdog.join()
