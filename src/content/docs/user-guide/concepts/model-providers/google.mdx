...
The Google provider uses the native `models.count_tokens()` API for message content. However, the Gemini API does not support counting system instructions or tool specifications natively — these are estimated separately using a character-based heuristic (characters ÷ 4 for text, characters ÷ 2 for JSON).
...
The Google provider uses the native `models.countTokens()` API for message content. However, the Gemini API does not support counting system instructions or tool specifications natively — these are estimated separately using a character-based heuristic (characters ÷ 4 for text, characters ÷ 2 for JSON).
...
When disabled (or if the API call fails), falls back to the character-based heuristic for all content.
</Tab>
</Tabs>
