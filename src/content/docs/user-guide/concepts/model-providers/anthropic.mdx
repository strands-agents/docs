...
The Anthropic provider uses the native `messages.count_tokens()` API, which provides exact token counts including system prompts, messages, and tool specifications.
...
When disabled (or if the API call fails), falls back to a character-based heuristic (characters ÷ 4 for text, characters ÷ 2 for JSON).
</Tab>
<Tab label="TypeScript">

The Anthropic provider uses the native `messages.countTokens()` API, which provides exact token counts including system prompts, messages, and tool specifications.
