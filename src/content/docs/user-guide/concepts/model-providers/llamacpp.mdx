...
The llama.cpp provider uses the server's native `/tokenize` endpoint with chat-template-aware tokenization (requires a llama.cpp server version that supports the `messages` field in tokenize requests). This provides accurate token counts that account for the model's chat template.
...
When disabled (or if the server doesn't support message-based tokenization), falls back to a character-based heuristic (characters ÷ 4 for text, characters ÷ 2 for JSON).
