...
The OpenAI Responses provider uses the native `responses.input_tokens.count()` API, which provides exact token counts including messages, instructions, and tool specifications.
...
The OpenAI Responses provider does not currently implement native token counting in the TypeScript SDK. It uses a character-based heuristic (characters ÷ 4 for text, characters ÷ 2 for JSON).
</Tab>
</Tabs>
