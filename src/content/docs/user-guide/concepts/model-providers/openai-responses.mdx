...
:::note
Python requires `openai>=2.0.0`. Install or upgrade with `pip install -U openai`.
:::
...
When using a stateful model, the agent automatically uses a null conversation manager and throws an error if a conversation manager is also supplied. In TypeScript, the agent's `modelState` property stores provider-specific data (such as the response ID) and is included in snapshots for session persistence.
