...
In TypeScript, the agent card is fetched and cached internally on the first `invoke()` or `stream()` call. There is no separate public method to retrieve it.
...
:::note[TypeScript Graph support]
`A2AAgent` in Graph workflows is not yet supported in TypeScript. The `GraphNode` currently requires a concrete `Agent` instance. Support for `AgentBase` (which `A2AAgent` implements) is planned for a future release.
:::
...
Both implementations serve the agent card at `/.well-known/agent-card.json` and handle JSON-RPC requests at the root path. Streaming is supported by default.
