...
:::note[TypeScript]
`A2AAgent` in Graph workflows is not yet supported in the TypeScript SDK. Graph support for `AgentBase` (which `A2AAgent` implements) is planned for a future release.
:::
