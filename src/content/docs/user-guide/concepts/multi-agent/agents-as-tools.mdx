...
By default, both direct passing and `.asTool()` reset the agent's conversation context between invocations, ensuring every call starts from a clean baseline. To preserve the agent's conversation history across invocations, use `.asTool()` with `preserveContext: true`:
