...
When streaming events are serialized (for example, with `JSON.stringify()`), each event keeps its `type` discriminator along with the relevant data fields for that event. This enables compact output for SSE, WebSockets, or HTTP responses.
...
| Event | Serialized fields |
|---|---|
| `NodeStreamUpdateEvent` | `{ type, nodeId, nodeType, inner }` |
| `NodeResultEvent` | `{ type, nodeId, nodeType, result }` |
| `MultiAgentHandoffEvent` | `{ type, source, targets }` |
| `NodeCancelEvent` | `{ type, nodeId, message }` |
| `BeforeNodeCallEvent` | `{ type, nodeId }` |
| `AfterNodeCallEvent` | `{ type, nodeId, error? }` |
| `MultiAgentResultEvent` | `{ type, result }` |
| Lifecycle events (`MultiAgentInitializedEvent`, `BeforeMultiAgentInvocationEvent`, `AfterMultiAgentInvocationEvent`) | `{ type }` |

**A2A events**

| Event | Serialized fields |
|---|---|
| `A2AStreamUpdateEvent` | `{ type, event }` |
| `A2AResultEvent` | `{ type, result }` |

Fields like `agent`, `orchestrator`, `state`, `tool`, `cancel`, and `retry` are not included in the serialized output since they hold large internal references — they remain accessible for direct programmatic use in-process. `Error` objects are converted to `{ message: string }`.

```typescript
for await (const event of agent.stream('Hello')) {
  // Each event serializes to compact JSON automatically
