...
Python streaming events are dictionaries and serialize directly with `json.dumps()`.

</Tab>
<Tab label="TypeScript">

When streaming events are serialized (e.g., via `JSON.stringify()`), each event keeps its `type` discriminator along with the relevant data fields for that event. This enables compact output for SSE, WebSockets, or HTTP responses:
...
Fields like `agent`, `tool`, `cancel`, and `retry` are not included in the serialized output since they hold large internal references — they remain accessible for direct programmatic use in-process.
