...
[Datadog AI Guard](https://docs.datadoghq.com/security/ai_guard/) is a defense-in-depth security solution that inspects, blocks, and governs AI behavior in real time. This integration connects AI Guard with Strands agents through the [Hooks](../../../user-guide/concepts/agents/hooks.md) system, providing inline security protection for your agent workflows.
...
The integration is provided by [`ddtrace`](https://github.com/DataDog/dd-trace-py) through the `AIGuardStrandsHookProvider` class, which implements the Strands `HookProvider` interface. It registers callbacks for four agent lifecycle events:
...
Each callback converts Strands messages into AI Guard's format and calls the AI Guard API to evaluate them against your configured security policies. If a threat is detected, the hook blocks or sanitizes the content before it reaches the model or the user.
...
Import the `AIGuardStrandsHookProvider` and pass it to your Strands agent:
...
The `AIGuardStrandsHookProvider` constructor accepts the following parameters:
...
```python
provider = AIGuardStrandsHookProvider(
    detailed_error=True,
    raise_error_on_tool_calls=True,
)

agent = Agent(hooks=[provider])
```
...
- [Strands Hooks documentation](../../../user-guide/concepts/agents/hooks.md)
