...
---
### AWS Lambda Durable — Agent as a Single Durable Step

AWS Lambda Durable Functions (launched December 2025) supports wrapping any callable as a `@durable_step`. You can wrap `agent("prompt")` as a single step today — **no SDK changes required**.
...
def run_strands_agent(ctx, prompt: str, session_id: str) -> str:
    agent = Agent(
        ...
        session_manager=S3SessionManager(
            bucket="state", session_id=session_id
        ),
    )
...
def handler(event: dict, context: DurableContext) -> dict:
    # Option A: Pass function + args separately
    result = context.step(
        run_strands_agent, event["prompt"], event["session_id"]
    )
    # ctx is injected automatically by the framework
...
However, the fundamental problem is still the same as the Temporal pattern. The entire agent loop is treated as one step. Crash recovery only happens at step boundaries, so if Lambda dies mid-loop, the entire step retries, and already-completed tool calls end up re-executing.
...
More to read [AWS Durable](https://github.com/aws/aws-durable-execution-sdk-python)


Our existing functions are untouched. The new durable function shares the same Layer, the thing we need to add is `aws_durable_execution_sdk_python` in `requirements.txt` and the `@durable_execution` / `@durable_step` decorators in the new handler.
...
The key takeaway is both of them need the handler to own the loop. They can't checkpoint what they can't see. If the agent loop is hidden inside our SDK, these platforms have no way to hook into individual steps.

**Temporal needs:** Each LLM call and each tool call as a separate Activity. Temporal records each Activity result permanently. On crash, completed Activities are replayed from history; the loop resumes at the step that was interrupted.

**Lambda Durable needs:** Each LLM call and each tool call wrapped in its own `context.step()`. AWS records each step result. On Lambda interruption, completed steps are skipped; the loop resumes at the step that was interrupted.
...
**The current SDK owns the loop inside `invoke_async`.** There is no mechanism for external code to inject a checkpoint between iterations. Event loop cannot call back into the durable platform between steps.

This is the single root cause of all current integration limitations. We can categorize them to be :
...
**1. Async hooks:** `HookRegistry` gains `invoke_callbacks_async()` so hooks can `await` external calls. The event loop uses this at `AfterToolCallEvent` and `AfterModelCallEvent`, which are the two moments a durable platform needs to write a checkpoint.
...
**2. `AgentSpec`:** A frozen, JSON-safe dataclass (`model_id`, `tool_names`, `tool_schemas`, `system_prompt`, …) built via `Agent._build_spec()` at dispatch time. This is the only thing that crosses the process boundary to a remote worker or Lambda handler, never a live `Agent` object.
...
| Gap                           | Fix                                                     | Where                     |
|-------------------------------|---------------------------------------------------------|---------------------------|
| Sync-only hooks               | `HookRegistry.invoke_callbacks_async()`                 | Core SDK                  |
...
### After Native Integration: What Changes

|                               | Today     |  2.0                  |
|-------------------------------|-----------|-----------------------------|
| Mid-loop crash recovery       | ❌         | ✅                           |
| Non-idempotent tools safe     | ❌         | ✅                           |
| Per-LLM / per-tool visibility | ❌         | ✅                           |
| `agent("prompt")` unchanged   | ✅         | ✅                           |
| AWS Lambda ceiling solved     | ✅         | ✅                           |
| SDK changes required          | None      | Core SDK + provider package |
...
|---|------|-------|------------|
| 1 | Update `event_loop.py` to call `invoke_callbacks_async()` at `AfterToolCallEvent` and `AfterModelCallEvent` | Core team | — |
| 2 | Add `AgentSpec` frozen dataclass and `Agent._build_spec()` | Core team | Stateless agent (@Patrick) |
| 3 | Add `DurableBackend` + `ExecutionHandle` ABCs in `strands.agent.backends` | Core team | #2 |
| 4 | Add `durable_backend` param and `dispatch_async()` to `Agent` | Core team | #3 |
| 5 | Add `ToolRegistry.resolve(name)` + `all_registered()` | Core team | — |
| 6 | Implement `strands-temporal` provider package | TBD | #1 #2 #3 #4 #5 |
| 7 | Implement `strands-aws` provider package | TBD | #1 #2 #3 #4 #5 |

## Willingness to Implement

Yes. The four core SDK gaps are the prerequisite and are owned by the core team. The `strands-temporal` and `strands-aws` provider packages are parallel efforts, each depending on the stateless agent proposal landing first.
